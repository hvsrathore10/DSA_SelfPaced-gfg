// Find strongly connected components in a given directed graph using 
...
// stackMember[] --> index array for faster check whethe a node is in stack
void Graph::SCCUtil(int u,int disc[],int low[],stack<int>* st,bool stackMember[]) {
	...
	for (i = adj[u].begin(); i != adj[u].end(); ++i) {
		...
		if (disc[v] == -1) {
			...
			// connection to one of the ancestors of 'u' case 1
			low[u] = min(low[u], low[v]);
		}

		// Update low value of 'u' only of 'v' is still in
		// stack (i.e. it's a back edge, not cross edge) case 2 
